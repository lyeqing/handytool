...
export default async function Home({ params }: PageProps<"/[lang]">) {
  ...
  // The API resolves every label into this language before returning it, so nothing below has to
  // know that translations exist at all.
  const result = await listDefinitions(locale);

  return (
    <div className="mx-auto w-full max-w-3xl px-6 py-10">
      <h1 className="text-2xl font-semibold tracking-tight">{t.site.title}</h1>
      <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
        {t.site.tagline}
      </p>

      <Link
        href={`/${locale}/schemas/new`}
      ...
      <div className="mt-8">
        {!result.ok ? (
          <p className="rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-200">
            {result.status === 401 ? t.errors.signIn : result.message}
          </p>
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="truncate text-xs text-zinc-500 dark:text-zinc-400">
                    #{definition.id} ·{" "}
                    {definition.description || t.home.noDescription}
                  </p>
                </div>
                <Link
                  href={`/${locale}/schemas/${definition.id}/records/new`}
