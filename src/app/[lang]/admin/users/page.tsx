...
export default async function Page({params,searchParams}:{params:Promise<{lang:string}>;searchParams:Promise<{q?:string;skip?:string;companyId?:string;sort?:string}>}) {
 ...
 return <section>
  {company&&<Link className="admin-text-action mb-4" href={`/${lang}/admin/users`}>← {t.allUsers}</Link>}
...
<button className="btn-secondary">{t.applyFilters}</button></form>
  ...
  <div className="grid grid-cols-1 items-start gap-5 lg:grid-cols-2 2xl:grid-cols-3">{result.data.items.map(row=><article key={row.id} className="admin-account-card min-w-0">
   <div className="admin-card-heading">
    ...
    <h3 className="break-words text-xl font-semibold tracking-tight text-slate-900">{row.displayName}</h3>
    <p className="mt-2 break-all text-sm text-slate-600">{row.email}</p>
    <div className="mt-4 flex flex-wrap gap-2">
     <span className="admin-status bg-white text-sky-800">{row.companyId? t.companyAccount:t.personalAccount}</span>
     {row.companyRole&&<span className="admin-status bg-sky-100 text-sky-800">{row.companyRole==="Owner"?t.owner:row.companyRole==="Admin"?t.admin:t.member}</span>}
     {row.isSuperAdmin&&<span className="admin-status bg-violet-100 text-violet-800">{t.isSuperAdmin}</span>}
    </div>
   </div>
   ...
   <div className="admin-card-actions"><details><summary className="admin-disclosure text-slate-700"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="size-5 shrink-0"><path d="m15 5 4 4M4 20l4-1L20 7a2 2 0 0 0-4-4L4 15Z"/></svg><span>{t.editUser}</span><span aria-hidden="true" className="admin-chevron ml-auto">⌄</span></summary>
