...
export default async function Page({params,searchParams}:{params:Promise<{lang:string}>;searchParams:Promise<{q?:string;skip?:string;sub?:string}>}) {
 const {lang}=await params; const query=await searchParams; const t=adminCopy(lang); const section="categories" as const;
 ...
 <div className="my-4 flex gap-4"><Link className="text-sky-700 underline" href={`/${lang}/admin/categories`}>{t.master}</Link><Link className="text-sky-700 underline" href={`/${lang}/admin/categories?sub=true`}>{t.sub}</Link></div><h3 className="my-4 font-medium">{sub?t.sub:t.master}</h3><details className="my-5 rounded-xl border border-sky-200 bg-sky-50 p-5"><summary className="cursor-pointer font-medium">{t.create}</summary><AdminForm lang={lang} section={section} sub={sub}/></details>
